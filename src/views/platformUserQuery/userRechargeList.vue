<template>
  ...
  <div class="app-container">
    ...
    <el-table :data="listData" v-loading="isComplet" element-loading-text="拼命加载中" border fit stripe highlight-current-row>
      ...
      <el-table-column label='手机号码' :min-width="150">
        <template slot-scope="scope">
          {{scope.row.mobile}}
        </template>
      </el-table-column>
      ...
      <el-table-column label='充值时间' :min-width="150">
        <template slot-scope="scope">
          {{scope.row.createDate | initTime}}
        </template>
      </el-table-column>
      <el-table-column label='充值金额' prop="balance" :min-width="150"></el-table-column>
...
export default {
  ...
  methods: {
    changeTime(v){
      this.pages.page = 1
      // console.log(this.formData.timer);
    ...
    getData() {
      userRechargeList(this.pages).then(res => {
