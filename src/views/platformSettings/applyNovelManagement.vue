<template>
  ...
  <div class="app-container">
    ...
    <el-table :data="applyBookList" element-loading-text="拼命加载中" border fit stripe highlight-current-row>
      ...
      <el-table-column label="联系方式类型" prop='contactType' :min-width="150">
        <template slot-scope="scope">
      ...
      <el-table-column label="联系方式内容" prop='contact' :min-width="150"></el-table-column>
      ...
      <el-table-column label="处理状态" :min-width="150">
        <template slot-scope="scope">
          ...
          <el-tag type="success" v-if="scope.row.state == 1">已处理已添加</el-tag> 
          <el-tag type="warning" v-if="scope.row.state == 2">已处理未添加</el-tag> 
        </template>
      </el-table-column>
